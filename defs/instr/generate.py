...
def validate(opcodes):
  ...
  for opname, oe in opcodes.items():
    ...
    for sname, se in oe["sub_opcodes"].items():
        sv = se["value"]
        if sv in sub_seen:
            errors.append(f"  SUB COLLISION in '{opname}': 0x{sv:02X} — '{sub_seen[sv]}' and '{sname}'")
        sub_seen[sv] = sname
  return errors
...
def entry_vars(name, value, mnemonic, description, extra=None):
  v = {
...
def apply_vars(text, d):
  # Handle {{#key}}...{{/key}} conditional blocks — render if value is truthy
  def cond_rep(m):
      key, inner = m.group(1), m.group(2)
      val = str(d.get(key, ""))
      return apply_vars(inner, d) if val and val not in ("false", "0", "") else ""
  text = re.sub(r"\{\{#(\w+)\}\}(.*?)\{\{/\1\}\}", cond_rep, text, flags=re.DOTALL)
...
def render_block(tmpl, items):
  return "".join(apply_vars(tmpl, v) for v in items)

def op_vars(name, e):
  return entry_vars(name, e["value"], e["mnemonic"], e["description"], {
...
def render(template, meta, opcodes, aliases):
  ...
  sub(r"\{\{#opcodes\}\}(.*?)\{\{/opcodes\}\}",
      lambda m: render_block(m.group(1), [op_vars(n, e) for n, e in by_value]))

  sub(r"\{\{#paired_opcodes\}\}(.*?)\{\{/paired_opcodes\}\}",
      lambda m: render_block(m.group(1), [op_vars(n, e) for n, e in by_value if e["pair"]]))

  sub(r"\{\{#unpaired_opcodes\}\}(.*?)\{\{/unpaired_opcodes\}\}",
      lambda m: render_block(m.group(1), [op_vars(n, e) for n, e in by_value if not e["pair"]]))

  def flat_subs(m):
      items = []
      for opname, oe in by_value:
          for sname, se in sorted(oe["sub_opcodes"].items(), key=lambda x: x[1]["value"]):
              items.append(entry_vars(sname, se["value"], sname, se["description"], {
                  "parent_name": opname, "PARENT_UPPER": upper(opname), "ParentPascal": pascal(opname),
              }))
      return render_block(m.group(1), items)
  sub(r"\{\{#sub_opcodes\}\}(.*?)\{\{/sub_opcodes\}\}", flat_subs)
  ...
  def nested_subs(m):
      ...
      for opname, oe in by_value:
          ...
          if em:
              evars = [dict(entry_vars(sn, se["value"], sn, se["description"]), **pv)
                        for sn, se in sorted(oe["sub_opcodes"].items(), key=lambda x: x[1]["value"])]
              inner = inner[:em.start()] + render_block(em.group(1), evars) + inner[em.end():]
          parts.append(apply_vars(inner, pv))
      return "".join(parts)
  sub(r"\{\{#sub_opcode_parents\}\}(.*?)\{\{/sub_opcode_parents\}\}", nested_subs)

  def alias_block(m):
      items = []
      for aname, ae in sorted(aliases.items()):
          target = opcodes.get(ae["alias_of"], {})
          items.append(entry_vars(aname, target.get("value", 0), aname, ae["description"], {
              "alias_of": ae["alias_of"], "ALIAS_UPPER": upper(ae["alias_of"]),
          }))
      return render_block(m.group(1), items)
  sub(r"\{\{#aliases\}\}(.*?)\{\{/aliases\}\}", alias_block)

  # Scalar meta last
  scalars = {k: str(v) for k, v in meta.items() if not k.startswith("_")}
  scalars["date"] = date.today().isoformat()
  return apply_vars(text, scalars)
...
def main():
  ...
  sub_count = sum(len(e["sub_opcodes"]) for e in opcodes.values())
  print(f"Validation OK — {len(opcodes)} opcodes ({pairs} I/R pairs), "
        f"{sub_count} sub-opcodes, {len(aliases)} aliases.")

  if args.check:
      sys.exit(0)

  all_targets = loadTargets()
  targets = {args.target: all_targets[args.target]} if args.target else all_targets
  OUT_DIR.mkdir(exist_ok=True)

  for lang, cfg in targets.items():
      tmpl_path = TMPL_DIR / cfg["template"]
      if not tmpl_path.exists():
          print(f"  SKIP {lang}: '{cfg['template']}' not found")
          continue
      content = render(tmpl_path.read_text(), meta, opcodes, aliases)
      out_path = OUT_DIR / cfg["output"]
      out_path.write_text(content)
      print(f"  wrote {out_path.relative_to(ROOT)}  ({lang})")
