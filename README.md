...
- `defs/`: holds headers and other definitions to be used by projects. Currently present are `aoef.h` and `aoef.v`, which provide definitions and macros for the AOE file format, and `pcre2.h` (the PCRE2 regular expression header).
...
- `utf8b`: a C-facing UTF-8 helper library that re-exports V language UTF-8 functionality for use from C (uses V source code).
